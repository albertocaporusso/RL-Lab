...
class MultiArmedBandit():
	...
	def __init__( self, levers ):
		...
		# at each action can be assigned a value q*(a) sampled form a normal distribution
		# with mean = 0 and variance = 1
		self.q_star = list(np.random.normal(0,self.sampling_variance,levers)) #0 is the mean and levers is the number of value to sample

	def action( self, action ):
		# for a given action we return a sempled reward from mean = q*(action) and variance = 1
...
def banditAlgorithm( env, eps=0, maxiters=1000 ):
	...
	N = np.array([0 for _ in range(levers) ], dtype=int) #how many times and action is selected
	ep_reward = []; avg_reward = []

	for _ in range(maxiters):

		random_action = int(np.random.rand(1)*env.levers) #sample a number between 0 and levers
