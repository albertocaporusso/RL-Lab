...
def q_learning(environment, episodes, alpha, gamma, expl_func, expl_param):
	...
	for i in range(episodes):
		...
		while not environment.is_terminal(s) :
			...
			r = environment.R[s_1]
			q[s][a] = q[s][a] + alpha*(r+gamma*max(q[s_1]) - q[s][a]) #a_1 è scelta off-policy ossia come il max sulla Q-function
			s = s_1
			rews[i]+=r
			lengths[i]+=1
...
def sarsa(environment, episodes, alpha, gamma, expl_func, expl_param):
	...
	for i in range(episodes) :
		...
		while True:
			s_1 = environment.sample(a,s)
			r = environment.R[s_1]
			a_1 = expl_func(q, s_1, expl_param) #a_1 è scelta in base alla policy
			q[s][a] = q[s][a] + alpha*(r + gamma*q[s_1][a_1] - q[s][a])
			rews[i]+=r
			lengths[i]+=1
			if environment.is_terminal(s_1): break
			s = s_1
			a = a_1
